    ...
    public function getCouponDataAttribute()
    {
        if (empty($this->_couponData)) {
            ...
            $this->_couponData = collect($this->couponList)->map(function ($coupon) use ($goods, $storeId, $money, $selectId, $scene) {
                ...
                if ($coupon['storeType'] == 2 && !in_array($storeId, $coupon['coupon']['storeIds'])) {
                    $coupon['useState'] = 'false';
                    $coupon['msg'][] = '当前门店不可使用';
                }
                ...
                if ($coupon['storeType'] == 3 && in_array($storeId, $coupon['coupon']['storeIds'])) {
                    $coupon['useState'] = 'false';
                    $coupon['msg'][] = '当前门店不可使用';
                }
                if (($coupon['coupon']['type'] == 4 && $this->scene != 1) || ($coupon['coupon']['type'] == 4 && $this->carList->deliveryMoney['money'] == 0)) {
                    $coupon['useState'] = 'false';
                    $coupon['msg'][] = '订单无配送费';
                }
                if ($coupon['coupon']['goodsType'] == 2) {
                    $goods = collect($goods)->where('discountType', 0)->whereIn('spuId', $coupon['coupon']['goodsIds'])->all();
                    if ($coupon['coupon']['rule']['disType'] == 5) {
                        $goods = collect($goods)->where('discountType', 0)->whereIn('spuId', $coupon['coupon']['goodsIds'])->all();
                    }
                }
                if ($coupon['coupon']['goodsType'] == 3) {
                    $goods = collect($goods)->where('discountType', 0)->whereNotIn('spuId', $coupon['coupon']['goodsIds'])->all();
                    if ($coupon['coupon']['rule']['disType'] == 5) {
                        $goods = collect($goods)->where('discountType', 0)->whereNotIn('spuId', $coupon['coupon']['goodsIds'])->all();
                    }
                }
                if ($coupon['coupon']['type'] == 5) {
                    if ($coupon['coupon']['rule']['disType'] == 1) {
                        $goods = [collect($goods)->where('discountType', 0)->sortByDesc('price')->first()];
                    } else {
                        $goods = [collect($goods)->where('discountType', 0)->sortBy('price')->first()];
                    }
                }

                $goodsMoney = collect($goods)->sum('money');
                if (in_array($coupon['coupon']['goodsType'],[2,3])&&$goodsMoney == 0) {
                    $coupon['useState'] = 'false';
                    $coupon['msg1'][] = '当前商品不可使用';
                }
                $money = $goodsMoney;
                $materialMoney = collect($goods)->sum('materialMoney');
                $boxMoney = collect($goods)->sum('boxMoney');
                if (!in_array(1, $coupon['coupon']['threshold'] ?? [0])) {
                    $money = bcsub($money, $materialMoney ?? 0);
                }
                if (in_array(1, $coupon['coupon']['threshold'] ?? [0])) {
                    $money = bcadd($money, $boxMoney ?? 0);
                }
                if (in_array(1, $coupon['coupon']['threshold'] ?? [0])) {
                    $money = bcadd($money, $this->carList->deliveryMoney['money'] ?? 0);
                }
                ...
                if ($coupon['coupon']['type'] == 5) {
                    ...
                    $coupon['money'] = bcsub($goods->price, $coupon['coupon']['rule']['money'], 2);
                }
                $coupon['select'] = 0;
