...
class CartList extends BaseModel
{
    ...
    {
        if (empty($this->_newSub) && !$this->tables->orderSn) {
        ...
        return $discount->money ? $discount->money > $this->goodsMoney ? $this->goodsMoney : $discount->money : null;
    }
    ...
    public function getDiscountsAttribute()
    {
        ...
        if (empty($this->_discounts)) {
            ...
            if ($this->vipDiscount) {
                $this->_discounts['vipDiscount'] = ['activityId' => 0, 'activityName' => "会员{$this->user->vip->discount}折", 'type' => 'vipDiscount', 'money' => $this->vipDiscount, 'title' => "会员折扣"];
            }
            // if ($this->vipFreeMail) {
