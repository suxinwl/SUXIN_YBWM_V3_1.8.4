...
class ChannelCart extends BaseModel
{
    ...
    public $_sku;
    protected $fillable = [
        'equityCardMemberId', 'isTemp', 'tempIndex', 'name', 'pack', 'setMealData', 'spuId', 'price', 'money', 'sellMoney', 'boxMoney', 'boxPrice', 'specMd5', 'attrData', 'materialData', 'tableId', 'diningType', 'score', 'adminId', 'notes', 'reason', 'discountNum', 'discountPrice', 'discountType', 'discountLabel', 'discountMoney'
    ];
    protected $casts =  [
        'attrData' => 'array',
        'materialData' => 'array',
    ...
    {
        ksort(collect($this->attrData)->toArray());
        if ($this->diningType == 4) {
            $str =  $this->discountType . intval($this->score) . intval($this->uniacid) . intval($this->storeId) . 'inStore' . intval($this->tableId) . $this->specMd5 . json_encode($this->attrData ?? [], 320);
        } else {
            $str = intval($this->score) . intval($this->adminId) . intval($this->uniacid) . intval($this->storeId) . intval($this->tableId) . 'inStore' . $this->specMd5 . json_encode($this->attrData ?? [], 320);
        }
        return md5($str);
    }
    ...
    public function check()
    {
        ...
        //     }
        //     $userLimitKey = "userGoods:" . $spu->spu->id . ":" . $this->userId;
        ...
        //     }
        //     $userDayLimitKey = "userGoods:" . date("Y-m-d") . ":" . $spu->spu->id . ":" . $this->userId;
    ...
    public function  model($inCart = true, $tableId='',$specMd5='',$setMealData='',$attrData='',$adminId=0, $disCountChange = true)
    {

        $specMd5=$specMd5?:$this->getMD5();

        if ($inCart) {
            $goods = ChannelCart::where('specMd5', $specMd5)
                ->where("uniacid", $this->uniacid)
                ->where("storeId", $this->storeId)
                ->where("diningType", $this->diningType);
            if($adminId){
                $goods->where("adminId",$adminId);
            }
            if($tableId){
        ...
        if (empty($goods)) {
            if($this->isTemp==1){
                ...
                $goods->sellMoney = bcmul($this->num,$this->price,2);
                $goods->boxPrice = 0;
                $goods->num = $this->num;
                $goods->setMealMoney = 0;
                $goods->materialMoney = 0;
                $goods->discountMoney =0;
                $goods->money = bcmul($this->num,$this->price,2);
                return $goods;
            }else{
                $sku = $this->sku;
                $spu = $this->spu;
        ...
        //     $goods->discountNum = 0;
        // }
        if (in_array($goods->discountType, [0, 6, 7, 8, 10])  && $goods->isTemp == 0) {
            $goods->getDiscount($disCountChange);
        }
    ...
    public function getDiscount($disCountChange = true)
    {
        $sku = $this->sku;
        if ($sku->discount && in_array($this->discountType, [0, 6, 7, 8, 10])) {
    ...
    {
        $goods = DB::table('goods_spu')->select(['id', 'name', 'logo', 'vipPriceSwitch'])->where('id', $this->spuId)->first();
        if (!$goods) {
