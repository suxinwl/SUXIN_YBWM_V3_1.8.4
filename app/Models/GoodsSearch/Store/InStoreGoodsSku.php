...
class InStoreGoodsSku extends BaseModel
{
    ...
    public function getDiscountAttribute()
    {
        if (!$this->_discount) {
            ...
            $discount = Goods::where('uniacid', $this->uniacid)
                ...
                ->when($diningType, function ($q) use ($diningType) {
            ...
            if ($discount) {
                ...
                if ($discount->type == 6) {
                    if ($discount->discountType == 1) {
                        $price = bcmul(bcdiv($price, 100, 4), $discount->dnDiscount * 10, 2);
                    }
                    if ($discount->discountType == 2) {
                        $price = bcsub($price, $discount->dnDiscount, 2);
                        if ($price < 0) {
                            $price = 0;
                        }
                    }
                    if ($discount->discountType == 3) {
                        $price = $discount->dnDiscount;
                        if ($price < 0) {
                            $price = 0;
                        }
                    }
