...
trait HelperTrait
{
    ...
    // 递归无线树状结构
    protected function menusTree($data, $pid = 0, $lev = 0, $key = "pid")
    {
        $arr = [];
        foreach ($data as $v) {
            ...
            if ($v[$key] == $pid) {
                ...
                $v['lev'] = $lev;
                $v['children'] = $this->menusTree($data, $v['id'], $lev + 1);
    ...
    // 递归无线树状结构 多元数组
    protected function getChildren($data, $pid = 0, $lev = 0, $key = "pid")
    {
        $arr = [];
        foreach ($data as $v) {
            ...
            if ($v[$key] == $pid) {
                $v['lev'] = $lev;
                $v['children'] = $this->getChildren($data, $v['id'], $lev + 1, $key);
                if (count($v['children']) <= 0) {
                    unset($v['children']);
                }
                $arr[] = $v;
