...
class XinyeMaxContent
{
    ...
    public static function facepayContents($order)
    {
        ...
        $content .= self::LR('订单号：', $order->orderSn, 48) . '<BR>';
        if($order->pickNo) {
            $content .= self::LR('流水号：', $order->pickNo, 48) . '<BR>';
        }
    ...
    static function feiStyle($arr, $fixType = 1, $note = '',$isB=true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if($length<=48){
                    if($isB==true){
                        $str .= '<HB><BOLD>' . self::LR($name, $rightContent, 48) . '</BOLD></HB><BR>';
                    }else{
                        $str .=self::LR($name, $rightContent, 48) . '<BR>';
                    }
                }else{
                    if($isB==true) {
                        $str .= '<HB><BOLD>' . mb_substr($name, 0, 48, 'utf-8') . '</BOLD></HB><BR>';
                        $str .= '<HB><BOLD>' . self::LR(mb_substr($name, 48, $length, 'utf-8'), $rightContent, 48) . '</BOLD></HB><BR>';
                    }else{
                    ...
                    if($goodsTitleNum==2){
                        ...
                        switch ($v['size']){
                            case 1;//18个空格
                                ...
                                break;
                            case 2;
                                $content .= '<WB>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],24).'</WB>';
                                break;
                            case 3;
                                $content .= '<HB>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],48).'</HB>';
                                break;
                            case 4;//2个空格
    ...
    public static function widen($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<WB><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,24).'</BOLD></WB>';
                }else{
                    $orderInfo.='<WB><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,24).'</BOLD></WB>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<WB><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',24).'</BOLD></WB>';
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<WB><BOLD>'.self::LR($name,' ×'.$num,24).'</BOLD></WB>';
                }else{
                    $orderInfo.='<WB><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,24).'</BOLD></WB>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function heighten($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<HB><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,48).'</BOLD></HB>';
                }else{
                    $orderInfo.='<HB><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,48).'</BOLD></HB>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<HB><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',48).'</BOLD></HB>';
                    }
                }
            }else{
                $maxNumber=24;
                ...
                $number=floor(bcdiv((48-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<HB><BOLD>'.self::LR($name,' ×'.$num,48).'</BOLD></HB>';
                }else{
                    $orderInfo.='<HB><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,48).'</BOLD></HB>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function turntable($fromTableId,$toTableId,$order,$operator=''){
        ...
        $content .='<HB><BOLD>'.self::LR('原桌位：', $from->area->name.'-'.$from->type->name.'-'.$from->name, 48) . '</BOLD></HB><BR>';
        $content .= '<HB><BOLD>'.self::LR('转桌位：', $to->area->name.'-'.$to->type->name.'-'.$to->name, 48) . '</BOLD></HB><BR>';
