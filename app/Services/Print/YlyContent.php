...
class YlyContent
{
    ...
    public static function rechargeContents($order, $balance,$str='')
    {
        ...
        $content .= self::LR('订单号：', $order->orderSn, 32) . "\n";
        $content .= self::LR('下单时间：', date("m-d H:i", strtotime($order->created_at)), 32) . "\n";
        $content .= "********************************\n";
        $content .= self::LR('充值门店：', $order->store->name, 32) . "\n";
        $content .= self::LR('手机号码：', $order->user->mobile, 32) . "\n";
        $content .= self::LR('会员姓名：', $name, 32) . "\n";
        $content .= self::LR('充值金额：', $order->subOrder->money?:$order->money, 32) . "\n";
        $content .= self::LR('支付方式：', '微信支付', 32) . "\n";
        if($str){
            $content .= '充值赠送：' . $str . "\n";
        }
        $content .= self::LR('充后余额：', $balance, 32) . "\n";
        return $content;
    }
    //当面付订单
    public static function facepayContents($order)
    {
        ...
        $content .= self::LR('订单号：', $order->orderSn, 32) . "\n";
        if($order->pickNo) {
            $content .= self::LR('流水号：', $order->pickNo, 32) . "\n";
        }
        $content .= self::LR('支付时间：', date("m-d H:i", strtotime($order->created_at)), 32) . "\n";
        $content .= "********************************\n";
        ...
        $content .= self::LR('消费门店：', $order->store->name, 32) . "\n";
        $content .= self::LR('付款金额：', $order->money ?? $order->subOrder->money, 32) . "\n";
        $content .= self::LR('用户余额：', $account->balance, 32) . "\n";
        $content .= self::LR('支付方式：', $order->PayTypeFormat, 32) . "\n";
        return $content;
    }
    ...
    static function feiStyle($arr, $fixType = 1, $note = '',$isB=true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if($length<=32){
                    if($isB==true){
                        $str .= "<FH><FB>" . self::LR($name, $rightContent, 32) . "</FB></FH>\n";
                    }else{
                        $str .=self::LR($name, $rightContent, 32) . "\n";
                    }
                }else{
                    if($isB==true) {
                        $str .= "<FH><FB>" . mb_substr($name, 0, 32, "utf-8") . "</FB></FH>\n";
                        $str .= "<FH><FB>" . self::LR(mb_substr($name, 32, $length, "utf-8"), $rightContent, 32) . "</FB></FH>\n";
                    }else{
                        $str .= mb_substr($name, 0, 32, "utf-8") . "\n";
                        $str .= self::LR(mb_substr($name, 32, $length, "utf-8"), $rightContent, 32) . "\n";
                    }
                }
            }else{
                $rightContent=$nums;
    ...
    public static function instoreOneContents($order, $orderGoods = [], $otherMode = '')
    {
        ...
        if ($orderGoods) {
            ...
            $content .= "<FS2>" . self::LR($name, $orderGoods->num, 32) . "</FS2>";
            $content.="\n";
        } else {
        ...
        $content .= self::LR('单号：', $order->orderSn, 32) . "\n";
        $content .= self::LR('操作人：', $order->user->nickname, 32) . "\n";
        $content .= self::LR('时间：', date("Y-m-d H:i", strtotime($order->created_at)), 32) . "\n";
        return $content;
    }
...
//        }
        ...
        foreach ($data as $k=>$v){
            switch ($v['label']){
                ...
                case 'goodsTitle';
                    ...
                    if($goodsTitleNum==2){
                        $content .= self::LR($goodsTitle[0], $goodsTitle[1], 32) . "\n";
                    }else{
                        switch ($v['size']){
                            case 1;//18个空格
                                if($print_type==11){
                                ...
                                break;
                            case 2;
                                $content .= '<FW>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],16).'</FW>';
                                break;
                            case 3;
                                $content .= '<FH>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],32).'</FH>';
                                break;
                            case 4;//2个空格
                                $content .= '<FS2>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],16).'</FS2>';
                                break;
                        }
                    }
                    break;
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                if($number>$name_length){
                    $orderInfo.="<FB>".self::LR($name,' ×'.$num.' '.$prices,32)."</FB>";
                }else{
                    $orderInfo.="<FB>".self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,32)."</FB>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.="<FB>".self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',32)."</FB>";
                    }
                }
            }else{
                $maxNumber=16;
                ...
                $number=floor(bcdiv((32-$right_length),2));
                if($number>$name_length){
                    $orderInfo.="<FB>".self::LR($name,' ×'.$num,32)."</FB>";
                }else{
                    $orderInfo.="<FB>".self::LR(mb_substr($name,0,$number),' ×'.$num,32)."</FB>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function widen($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.="<FW><FB>".self::LR($name,' ×'.$num.' '.$prices,16)."</FB></FW>";
                }else{
                    $orderInfo.="<FW><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,16)."</FB></FW>";
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.="<FW><FB>".self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',16)."</FB></FW>";
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.="<FW><FB>".self::LR($name,' ×'.$num,16)."</FB></FW>";
                }else{
                    $orderInfo.="<FW><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num,16)."</FB></FW>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function heighten($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.="<FH><FB>".self::LR($name,' ×'.$num.' '.$prices,32)."</FB></FH>";
                }else{
                    $orderInfo.="<FH><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,32)."</FB></FH>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.="<FH><FB>".self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',32)."</FB></FH>";
                    }
                }
            }else{
                $maxNumber=16;
                ...
                $number=floor(bcdiv((32-$right_length),2));
                if($number>$name_length){
                    $orderInfo.="<FH><FB>".self::LR($name,' ×'.$num,32)."</FB></FH>";
                }else{
                    $orderInfo.="<FH><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num,32)."</FB></FH>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function amplify($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.="<FS2><FB>".self::LR($name,' ×'.$num.' '.$prices,16)."</FB></FS2>";
                }else{
                    $orderInfo.="<FS2><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,16)."</FB></FS2>";
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.="<FS2><FB>".self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',16)."</FB></FS2>";
                    }
                }
            }else{
                $number=6;$maxNumber=8;
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.="<FS2><FB>".self::LR($name,' ×'.$num,16)."</FB></FS2>";
                }else{
                    $orderInfo.="<FS2><FB>".self::LR(mb_substr($name,0,$number),' ×'.$num,16)."</FB></FS2>";
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function refundContent($order, $orderGoods = [], $operator = '')
    {
        ...
        $content .= self::LR('单号：', $order->orderSn, 32) . "\n";
        if ($operator) {
            $content .= self::LR('操作人：', $operator, 32) . "\n";
        }
        $content .= self::LR('时间：', date("Y-m-d H:i", strtotime($order->created_at)), 32) . "\n";
        return $content;
    }
    ...
    public static function turntable($fromTableId,$toTableId,$order,$operator=''){
        ...
        $content .='<FB><FH>'.self::LR('原桌位：', $from->area->name.'-'.$from->type->name.'-'.$from->name, 32) . "</FH></FB>\n";
        $content .= '<FB><FH>'.self::LR('转桌位：', $to->area->name.'-'.$to->type->name.'-'.$to->name, 32) . "</FH></FB>\n";
        $content .= self::LR('人数：', $order->people, 32) . "\n";
        $content .= self::LR('单号：', $order->orderSn, 32) . "\n";
        if($operator){
            $content .= self::LR('操作人：', $operator, 32) . "\n";
        }
        $content .= self::LR('时间：', date('Y-m-d H:i:s',time()), 32) . "\n";
        return $content;
    }
...
//        }
        ...
        $content .= "<C>交班汇总</C>";
        $content .= self::LR("本班次收款：",$handoverOrder->contents->money?:0.00,32)."\n";
        $content .= self::LR("微信收款：",$dataList->weixinMoney?:0.00,32)."\n";
        $content .= self::LR("支付宝收款：",$dataList->alipayMoney?:0.00,32)."\n";
        $content .= self::LR("现金收款：",$dataList->cashMoney?:0.00,32)."\n";
        $content .= self::LR("余额收款：",$dataList->balanceMoney?:0.00,32)."\n";
        $content .= "--------------------------------\n";
        $content .= "<C>收款统计</C>";
        ...
        if(!empty($handoverOrder->contents->warn)){
            ...
            foreach ($handoverOrder->contents->warn as $v){
                $content .=self::LR($v->name,$v->count,32)."\n";
            }
            $content .= "--------------------------------\n";
        }
