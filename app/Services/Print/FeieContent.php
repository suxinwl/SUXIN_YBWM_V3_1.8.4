...
class FeieContent
{
    ...
    static function feiStyle($arr, $fixType = 1, $note = '',$isB=true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if($length<=32){
                    if($isB==true){
                        $str .= '<L><BOLD>' . self::LR($name, $rightContent, 32) . '</BOLD></L><BR>';
                    }else{
                        $str .=self::LR($name, $rightContent, 32) . '<BR>';
                    }
                }else{
                    if($isB==true) {
                        $str .= '<L><BOLD>' . mb_substr($name, 0, 32, 'utf-8') . '</BOLD></L><BR>';
                        $str .= '<L><BOLD>' . self::LR(mb_substr($name, 32, $length, 'utf-8'), $rightContent, 32) . '</BOLD></L><BR>';
                    }else{
        ...
        foreach ($data as $k=>$v){
            switch ($v['label']){
                    ...
                    if($goodsTitleNum==2){
                        ...
                        switch ($v['size']){
                            ...
                            case 2;
                                ...
                                break;
                            case 3;
                                $content .= '<L>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],32).'</L>';
                                break;
                            case 4;//2个空格
                ...
                case 'payMoney';
                    ...
                    $payMoney=$order->service_money?bcadd($order->money,$order->service_money,2):$order->money;
                ...
                case 'disbursement';
                    if ($order->payTime) {
                        $disbursementMoney=$order->service_money?bcadd($order->money,$order->service_money,2):$order->money;
                    ...
                    break;
                case 'payType';//应付金额
                    $actualMoney=$order->service_money?bcadd($order->money,$order->service_money,2):$order->money;
                    $content.=self::customContent($v,$data[$k]['name'].'：'.$actualMoney).'<BR>';
                    break;

                case 'payAll';
                    $payAllMoney=$order->service_money?bcadd($order->money,$order->service_money,2):$order->money;
    ...
    public static function widen($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,16).'</BOLD></W>';
                }else{
                    $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,16).'</BOLD></W>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',16).'</BOLD></W>';
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><BOLD>'.self::LR($name,' ×'.$num,16).'</BOLD></W>';
                }else{
                    $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,16).'</BOLD></W>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function heighten($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<L><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,32).'</BOLD></L>';
                }else{
                    $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,32).'</BOLD></L>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',32).'</BOLD></L>';
                    }
                }
            }else{
                $maxNumber=16;
                ...
                $number=floor(bcdiv((32-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<L><BOLD>'.self::LR($name,' ×'.$num,32).'</BOLD></L>';
                }else{
                    $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,32).'</BOLD></L>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function turntable($fromTableId,$toTableId,$order,$operator=''){
        ...
        $content .='<L><BOLD>'.self::LR('原桌位：', $from->area->name.'-'.$from->type->name.'-'.$from->name, 32) . '</BOLD></L><BR>';
        $content .= '<L><BOLD>'.self::LR('转桌位：', $to->area->name.'-'.$to->type->name.'-'.$to->name, 32) . '</BOLD></L><BR>';
