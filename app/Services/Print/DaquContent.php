...
//https://gitee.com/jhbopenapi/juhebei-opendoc/tree/master/printer#%E5%B0%8F%E7%A5%A8%E5%86%85%E5%AE%B9%E6%8E%92%E7%89%88%E6%8E%A7%E5%88%B6
class DaquContent
{
    ...
    public static function rechargeContents($order, $balance, $str = '')
    {
        ...
        $content = "<C><font# bolder=0 height=2 width=2>充值订单</font#></C><BR><BR>";
    ...
    public static function facepayContents($order)
    {
        ...
        $name = $order->user->realname ?: $order->user->nickname;
        $content = "<C><font# bolder=0 height=2 width=2>当面付订单</font#></C><BR><BR>";
        $content .= self::LR('订单号：', $order->orderSn, 32) . '<BR>';
        if ($order->pickNo) {
    ...
    static function feiStyle($arr, $fixType = 1, $note = '', $isB = true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if ($length <= 32) {
                    if ($isB == true) {
                        $str .= "<font# bolder=0 height=1 width=1>" . self::LR($name, $rightContent, 32) . "</font#><BR>";
                    } else {
                        $str .= self::LR($name, $rightContent, 32) . '<BR>';
                    }
                } else {
                    if ($isB == true) {
                        $str .= "<font# bolder=0 height=1 width=1>" . mb_substr($name, 0, 32, "utf-8") . "</font#><BR>";
                        $str .= "<font# bolder=0 height=1 width=1>" . self::LR(mb_substr($name, 32, $length, "utf-8"), $rightContent, 32) . "</font#><BR>";
                    } else {
    ...
    public static function instoreOneContents($order, $orderGoods = [], $otherMode = '')
    {
        ...
        $content = "<C><font# bolder=0 height=2 width=2>" . $makeName . "</font#></C><BR>";
        if ($order->tableId) {
            if ($order->table->area->name) {
                $content .= "<font# bolder=0 height=1 width=1>桌号:" . $order->table->area->name . '-' . $order->table->name . "</font#><BR><BR>";
            } else {
                $content .= "<font# bolder=0 height=1 width=1>桌号:" .  $order->table->name . "</font#><BR><BR>";
            }

            $content .= "人数：" . $order->people . "<BR><BR>";
        } else {
            $content .= "<font# bolder=0 height=1 width=1>牌号:" . $order->pickNo . "</font#><BR>";
        }
        ...
        if ($orderGoods) {
            ...
            $content .= "<font# bolder=0 height=1 width=1>" . self::LR($name, $orderGoods->num, 32) . "</font#><BR>";
        } else {
            $content .= self::feiStyle($order->goods, 2) . '<BR>';
        }
        $content .= "--------------------------------<BR>";
        if ($order->notes) {
            $content .= "<font# bolder=0 height=1 width=1>整单备注：" . $order->notes . "</font#><BR>";
            $content .= "--------------------------------<BR>";
        }
    ...
    public static function instoreCustomerContents($order, $print_type = 1, $goods = [], $refundReason = '')
    {
        ...
        $template = PrintTemplate::where(function ($q) use ($order) {
            ...
            switch ($v['label']) {
                ...
                case 'goodsTitle';
                    ...
                    if ($goodsTitleNum == 2) {
                        ...
                        switch ($v['size']) {
                            case 1; //18个空格
                                if ($print_type == 11) {
                                    $content .= '<font# bolder="0" height="0" width="0">' . self::LR($goodsTitle[0], $goodsTitle[1] . '            ' . $goodsTitle[2], 32) . '</font#>';
                                } else {
                                    $content .= '<font# bolder="0" height="0" width="0">' . self::LR($goodsTitle[0], $goodsTitle[1] . '        ' . $goodsTitle[2], 32) . '</font#>';
                                }
                                break;
                            case 2;
                                $content .= '<font# bolder="0" height="0" width="1">' . self::LR($goodsTitle[0], $goodsTitle[1] . ' ' . $goodsTitle[2], 16) . '</font#>';
                                break;
                            case 3;
                                $content .= '<font# bolder="0" height="1" width="0">' . self::LR($goodsTitle[0], $goodsTitle[1] . ' ' . $goodsTitle[2], 32) . '</font#>';
                                break;
                            case 4; //2个空格
                                $content .= '<font# bolder="0" height="1" width="1">' . self::LR($goodsTitle[0], $goodsTitle[1] . ' ' . $goodsTitle[2], 16) . '</font#>';
                                break;
                        }
                    }
                    break;
                ...
                case 'packTime';
                    if ($order->scene == 1) {
                        ...
                        if ($order->appointment == 1) {
                            if (strtotime($order->serverTime) > strtotime(date("Y-m-d 23:59:59"))) {
                                $content .= self::customContent($v, $data[$k]['name'] . '：' . "预订单" . date("m-d H:i", strtotime($order->serverTime))) . '<BR>';
                                $content .= self::customContent($v, "取餐码：" . $order->pickNo) . '<BR>';
                            } else {
    ...
    public static function customContent($data, $content)
    {
        if ($data) {
            ...
            if ($data['size']) {
                switch ($data['size']) {
                    case 2;
                        $content = "<font# bolder=0 height=1 width=2>" . $content . "</font#><BR>";
                        break;
                    case 3;
                        $content = "<font# bolder=0 height=2 width=1>" . $content . "</font#><BR>";
                        break;
                    case 4;
                        $content = "<font# bolder=0 height=2 width=2>" . $content . "</font#><BR>";
                        break;
                }
            }
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($printType == 1) {
                ...
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=1 width=1>" . self::LR($name, ' ×' . $num . ' ' . $prices, 32) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=1 width=1>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num . ' ' . $prices, 32) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
                        $orderInfo .= "<font# bolder=0 height=1 width=1>" . self::LR(mb_substr($name, $number + $i * $maxNumber, $maxNumber), '', 32) . "</font#>";
                    }
                }
            } else {
                $maxNumber = 16;
                ...
                $number = floor(bcdiv((32 - $right_length), 2));
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=1 width=1>" . self::LR($name, ' ×' . $num, 32) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=1 width=1>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num, 32) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
    ...
    public static function widen($arr, $note = '', $printType = 1, $notes = '')
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($printType == 1) {
                ...
                $name_length = mb_strlen($name, 'utf-8');
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=1 width=2>" . self::LR($name, ' ×' . $num . ' ' . $prices, 16) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=1 width=2>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num . ' ' . $prices, 16) . "</font#>";
                }

                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
                        $orderInfo .= "<font# bolder=0 height=1 width=2>" . self::LR(mb_substr($name, $number + $i * $maxNumber, $maxNumber), '', 16) . "</font#>";
                    }
                }
            } else {
                $number = 6;
                ...
                $name_length = mb_strlen($name, 'utf-8');
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=1 width=2>" . self::LR($name, ' ×' . $num, 16) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=1 width=2>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num, 16) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
    ...
    public static function heighten($arr, $note = '', $printType = 1, $notes = '')
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($printType == 1) {
                ...
                $name_length = mb_strlen($name, 'utf-8');
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=2 width=1>" . self::LR($name, ' ×' . $num . ' ' . $prices, 32) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=2 width=1>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num . ' ' . $prices, 32) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
                        $orderInfo .= "<font# bolder=0 height=2 width=1>" . self::LR(mb_substr($name, $number + $i * $maxNumber, $maxNumber), '', 32) . "</font#>";
                    }
                }
            } else {
                $maxNumber = 16;
                ...
                $number = floor(bcdiv((32 - $right_length), 2));
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=2 width=1>" . self::LR($name, ' ×' . $num, 32) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=2 width=1>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num, 32) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
    ...
    public static function amplify($arr, $note = '', $printType = 1, $notes = '')
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($printType == 1) {
                ...
                $name_length = mb_strlen($name, 'utf-8');
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=2 width=2>" . self::LR($name, ' ×' . $num . ' ' . $prices, 16) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=2 width=2>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num . ' ' . $prices, 16) . "</font#>";
                }

                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
                        $orderInfo .= "<font# bolder=0 height=2 width=2>" . self::LR(mb_substr($name, $number + $i * $maxNumber, $maxNumber), '', 16) . "</font#>";
                    }
                }
            } else {
                $number = 6;
                ...
                $name_length = mb_strlen($name, 'utf-8');
                if ($number > $name_length) {
                    $orderInfo .= "<font# bolder=0 height=2 width=2>" . self::LR($name, ' ×' . $num, 16) . "</font#>";
                } else {
                    $orderInfo .= "<font# bolder=0 height=2 width=2>" . self::LR(mb_substr($name, 0, $number), ' ×' . $num, 16) . "</font#>";
                }
                $remainder = ceil((mb_strlen($name, 'utf-8') - $number) / $maxNumber);
                if ($remainder > 0) {
                    for ($i = 0; $i < $remainder; $i++) {
    ...
    {
        $content = "<C><font# bolder=0 height=2 width=2>转台单</font#></C>";
        $content .= "--------------------------------<BR>";
        $from = Table::with('type', 'area')->where('id', $fromTableId)->first();
        $to = Table::with('type', 'area')->where('id', $toTableId)->first();
        $content .= '<font# bolder=0 height=2 width=1>' . self::LR('原桌位：', $from->area->name . '-' . $from->type->name . '-' . $from->name, 32) . "</font#><BR>";
        $content .= '<font# bolder=0 height=2 width=1>' . self::LR('转桌位：', $to->area->name . '-' . $to->type->name . '-' . $to->name, 32) . "</font#><BR>";
    ...
    {
        $content = "<C><font# bolder=0 height=2 width=2>" . $handoverOrder->store->name . "</font#></C>";
        $content .= "<C><font# bolder=0 height=2 width=2>【交班单】</font#></C>";
        $content .= "交班班次：午班<BR>";
    ...
    public static function queuingNumber($queueOrder)
    {
        ...
        $content = '<C><font# bolder=1 height=2 width=2>' . $queueOrder->store->name . '</font#></C><BR>';
        $content .= "--------------------------------<BR>";
        $content .= '<C><font# bolder=1 height=2 width=2>' . $tableType . '</font#></C><BR>';
        $content .= '<C><font# bolder=1 height=2 width=2>' . $queueOrder->serialNum . '(' . $queueOrder->people . '人)</font#></C><BR>';
        $content .= '<C>前面还有' . $queueOrder->waitingTable . '桌</C><BR>';
        $content .= '<C>大约还需等待' . $queueOrder->waitingTime . '分钟</C><BR>';
        $content .= '<QR>' . $queueOrder->qrcode . '</QR><BOLD>还等几桌？扫码查进度</BOLD><BR>';
    ...
    public static function drinkLog($drinkLog)
    {
        ...
        if ($drinkLog->typeFormat) {
            $content = '<C><font# bolder=1 height=2 width=2>' . $drinkLog->typeFormat . '</font#></C><BR>';
            $content .= "--------------------------------<BR>";
        }
