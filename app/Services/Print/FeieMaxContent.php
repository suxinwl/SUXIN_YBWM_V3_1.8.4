...
class FeieMaxContent
{
    ...
    public static function rechargeContents($order, $balance,$str='')
    {
        ...
        $content .= self::LR('订单号：', $order->orderSn, 48) . '<BR>';
        $content .= self::LR('下单时间：', date('m-d H:i', strtotime($order->created_at)), 48) . '<BR>';
        $content .= '********************************<BR>';
        $content .= self::LR('充值门店：', $order->store->name, 48) . '<BR>';
        $content .= self::LR('手机号码：', $order->user->mobile, 48) . '<BR>';
        $content .= self::LR('会员姓名：', $name, 48) . '<BR>';
        $content .= self::LR('充值金额：', $order->subOrder->money?:$order->money, 48) . '<BR>';
        $content .= self::LR('支付方式：', '微信支付', 48) . '<BR>';
        if($str){
            $content .= '充值赠送：' . $str . '<BR>';
        }
        $content .= self::LR('充后余额：', $balance, 48) . '<BR>';
        return $content;
    }
    //当面付订单
    public static function facepayContents($order)
    {
        ...
        $content .= self::LR('支付时间：', date('m-d H:i', strtotime($order->created_at)), 48) . '<BR>';
        $content .= '********************************<BR>';
        ...
        $content .= self::LR('消费门店：', $order->store->name, 48) . '<BR>';
        $content .= self::LR('付款金额：', $order->money ?? $order->subOrder->money, 48) . '<BR>';
        $content .= self::LR('用户余额：', $account->balance, 48) . '<BR>';
        $content .= self::LR('支付方式：', $order->PayTypeFormat, 48) . '<BR>';
        return $content;
    }
    ...
    static function feiStyle($arr, $fixType = 1, $note = '',$isB=true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if($length<=48){
                    if($isB==true){
                        $str .= '<L><BOLD>' . self::LR($name, $rightContent, 48) . '</BOLD></L><BR>';
                    }else{
                        $str .=self::LR($name, $rightContent, 48) . '<BR>';
                    }
                }else{
                    if($isB==true) {
                        $str .= '<L><BOLD>' . mb_substr($name, 0, 48, 'utf-8') . '</BOLD></L><BR>';
                        $str .= '<L><BOLD>' . self::LR(mb_substr($name, 48, $length, 'utf-8'), $rightContent, 48) . '</BOLD></L><BR>';
                    }else{
                        $str .= mb_substr($name, 0, 48, 'utf-8') . '<BR>';
                        $str .= self::LR(mb_substr($name, 48, $length, 'utf-8'), $rightContent, 48) . '<BR>';
                    }
                }
            }else{
                $rightContent=$nums;
    ...
    public static function strFix($name, $A, $br = '<BR>', $maxLine = 48)
    {
    ...
    public static function instoreOneContents($order, $orderGoods = [], $otherMode = '')
    {
        ...
        if ($orderGoods) {
            ...
            $content .= '<B>' . self::LR($name, $orderGoods->num, 48) . '</B><BR>';
        } else {
            $content .= '<B>' . self::feiStyle($order->goods,2) . '</B><BR>';
        }
        $content .= '--------------------------------<BR>';
        if ($order->notes) {
            $content .= '<L><BOLD>整单备注：' . $order->notes . '</L></BOLD><BR>';
            $content .= '--------------------------------<BR>';
        }
        $content .= self::LR('单号：', $order->orderSn, 48) . '<BR>';
        $content .= self::LR('操作人：', $order->user->nickname, 48) . '<BR>';
        $content .= self::LR('时间：', date('Y-m-d H:i', strtotime($order->created_at)), 48) . '<BR>';
        return $content;
    }
        ...
        foreach ($data as $k=>$v){
            switch ($v['label']){
                ...
                case 'goodsTitle';
                    ...
                    if($goodsTitleNum==2){
                        $content .= self::LR($goodsTitle[0], $goodsTitle[1], 48) . '<BR>';
                    }else{
                        switch ($v['size']){
                            case 1;//18个空格
                                if($print_type==11){
                                ...
                                break;
                            case 2;
                                $content .= '<W>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],24).'</W>';
                                break;
                            case 3;
                                $content .= '<L>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],48).'</L>';
                                break;
                            case 4;//2个空格
                                $content .= '<B>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],24).'</B>';
                                break;
                        }
                    }
                    break;
...
//                    }
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                if($number>$name_length){
                    $orderInfo.='<BOLD>'.self::LR($name,' ×'.$num.' '.$prices,48).'</BOLD>';
                }else{
                    $orderInfo.='<BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,48).'</BOLD>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',48).'</BOLD>';
                    }
                }
            }else{
                $maxNumber=24;
                ...
                $number=floor(bcdiv((48-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<BOLD>'.self::LR($name,' ×'.$num,48).'</BOLD>';
                }else{
                    $orderInfo.='<BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,48).'</BOLD>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function widen($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,24).'</BOLD></W>';
                }else{
                    $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,24).'</BOLD></W>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',24).'</BOLD></W>';
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><BOLD>'.self::LR($name,' ×'.$num,24).'</BOLD></W>';
                }else{
                    $orderInfo.='<W><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,24).'</BOLD></W>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function heighten($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<L><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,48).'</BOLD></L>';
                }else{
                    $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,48).'</BOLD></L>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',48).'</BOLD></L>';
                    }
                }
            }else{
                $maxNumber=24;
                ...
                $number=floor(bcdiv((48-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<L><BOLD>'.self::LR($name,' ×'.$num,48).'</BOLD></L>';
                }else{
                    $orderInfo.='<L><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,48).'</BOLD></L>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function amplify($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<B><BOLD>'.self::LR($name,' ×'.$num.' '.$prices,24).'</BOLD></B>';
                }else{
                    $orderInfo.='<B><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,24).'</BOLD></B>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<B><BOLD>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',24).'</BOLD></B>';
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<B><BOLD>'.self::LR($name,' ×'.$num,24).'</BOLD></B>';
                }else{
                    $orderInfo.='<B><BOLD>'.self::LR(mb_substr($name,0,$number),' ×'.$num,24).'</BOLD></B>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function refundContent($order, $orderGoods = [], $operator = '')
    {
        ...
        $content .= self::LR('单号：', $order->orderSn, 48) . '<BR>';
        if ($operator) {
            $content .= self::LR('操作人：', $operator, 48) . '<BR>';
        }
        $content .= self::LR('时间：', date('Y-m-d H:i', strtotime($order->created_at)), 48) . '<BR>';
        return $content;
    }
    ...
    public static function turntable($fromTableId,$toTableId,$order,$operator=''){
        ...
        $content .='<L><BOLD>'.self::LR('原桌位：', $from->area->name.'-'.$from->type->name.'-'.$from->name, 48) . '</BOLD></L><BR>';
        $content .= '<L><BOLD>'.self::LR('转桌位：', $to->area->name.'-'.$to->type->name.'-'.$to->name, 48) . '</BOLD></L><BR>';
        $content .= self::LR('人数：', $order->people, 48) . '<BR>';
        $content .= self::LR('单号：', $order->orderSn, 48) . '<BR>';
        if($operator){
            $content .= self::LR('操作人：', $operator, 48) . '<BR>';
        }
        $content .= self::LR('时间：', date('Y-m-d H:i:s',time()), 48) . '<BR>';
        return $content;
    }
...
//        }
        ...
        $content .= "<C>交班汇总</C>";
        $content .= self::LR("本班次收款：",$handoverOrder->contents->money?:0.00,48)."<BR>";
        $content .= self::LR("微信收款：",$dataList->weixinMoney?:0.00,48)."<BR>";
        $content .= self::LR("支付宝收款：",$dataList->alipayMoney?:0.00,48)."<BR>";
        $content .= self::LR("现金收款：",$dataList->cashMoney?:0.00,48)."<BR>";
        $content .= self::LR("余额收款：",$dataList->balanceMoney?:0.00,48)."<BR>";
        $content .= '------------------------------------------------<BR>';
        $content .= "<C>收款统计</C>";
        ...
        if(!empty($handoverOrder->contents->warn)){
            ...
            foreach ($handoverOrder->contents->warn as $v){
                $content .=self::LR($v->name,$v->count,48)."<BR>";
            }
            $content .= '------------------------------------------------<BR>';
        }
...
//        $content .= '快餐            '.$handoverOrder->contents->unclosedFast->count.'单       '.$handoverOrder->contents->unclosedFast->money.'<BR>';
        $content .= '------------------------------------------------<BR><BR>';
        $content .= "交班人签字_______________<BR><BR>";
        $content .= "监督人签字_______________<BR>";
    ...
    public static function queuingNumber ($queueOrder){
        ...
        $content = '<C><B><BOLD>'.$queueOrder->store->name.'</BOLD></B></C><BR>';
        $content .= '------------------------------------------------<BR>';
