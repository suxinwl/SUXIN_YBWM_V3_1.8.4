...
class SpyunContent{
    ...
    static function feiStyle($arr, $fixType = 1, $note = '',$isB=true)
    {
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if ($fixType == 1) {
                ...
                if($length<=32){
                    if($isB==true){
                        $str .= '<H><B>' . self::LR($name, $rightContent, 32) . '</B></H><BR>';
                    }else{
                        $str .=self::LR($name, $rightContent, 32) . '<BR>';
                    }
                }else{
                    if($isB==true) {
                        $str .= '<H><B>' . mb_substr($name, 0, 32, 'utf-8') . '</B></H><BR>';
                        $str .= '<H><B>' . self::LR(mb_substr($name, 32, $length, 'utf-8'), $rightContent, 32) . '</B></H><BR>';
                    }else{
    ...
    public static function instoreCustomerContents($order,$print_type=1,$goods=[],$refundReason=''){
        ...
        foreach ($data as $k=>$v){
            switch ($v['label']){
                    ...
                    if($goodsTitleNum==2){
                        ...
                        switch ($v['size']){
                            case 1;//18个空格
                                ...
                                break;
                            case 2;
                                $content .= '<W>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],16).'</W>';
                                break;
                            case 3;
                                $content .= '<H>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],32).'</H>';
                                break;
                            case 4;//2个空格
                                $content .= '<L1>'.self::LR($goodsTitle[0],$goodsTitle[1].' '.$goodsTitle[2],16).'</L1>';
                                break;
                        }
                    }
                    break;
                ...
                case 'code';
                    if($data[$k]['value']==1){
                        $content.='<QRCODE>'.$order->qrcode.'</QRCODE>';
                    }
                    if($data[$k]['value']==2){
                        $content.='<QRCODE>'.$data[$k]['name'].'</QRCODE>';
                    }
                    break;
                case 'mobile';
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                if($number>$name_length){
                    $orderInfo.='<B>'.self::LR($name,' ×'.$num.' '.$prices,32).'</B>';
                }else{
                    $orderInfo.='<B>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,32).'</B>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<B>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',32).'</B>';
                    }
                }
            }else{
                $maxNumber=16;
                ...
                $number=floor(bcdiv((32-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<B>'.self::LR($name,' ×'.$num,32).'</B>';
                }else{
                    $orderInfo.='<B>'.self::LR(mb_substr($name,0,$number),' ×'.$num,32).'</B>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function widen($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><B>'.self::LR($name,' ×'.$num.' '.$prices,16).'</B></W>';
                }else{
                    $orderInfo.='<W><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,16).'</B></W>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<W><B>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',16).'</B></W>';
                    }
                }
            }else{
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<W><B>'.self::LR($name,' ×'.$num,16).'</B></W>';
                }else{
                    $orderInfo.='<W><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num,16).'</B></W>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function heighten($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<H><B>'.self::LR($name,' ×'.$num.' '.$prices,32).'</B></H>';
                }else{
                    $orderInfo.='<H><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,32).'</B></H>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<H><B>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',32).'</B></H>';
                    }
                }
            }else{
                $maxNumber=16;
                ...
                $number=floor(bcdiv((32-$right_length),2));
                if($number>$name_length){
                    $orderInfo.='<H><B>'.self::LR($name,' ×'.$num,32).'</B></H>';
                }else{
                    $orderInfo.='<H><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num,32).'</B></H>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    public static function amplify($arr,$note='',$printType=1,$notes=''){
        ...
        foreach ($arr as $k5 => $v5) {
            ...
            if($printType==1){
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<L1><B>'.self::LR($name,' ×'.$num.' '.$prices,16).'</B></L1>';
                }else{
                    $orderInfo.='<L1><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num.' '.$prices,16).'</B></L1>';
                }

                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
                        $orderInfo.='<L1><B>'.self::LR(mb_substr($name,$number+$i*$maxNumber,$maxNumber),'',16).'</B></L1>';
                    }
                }
            }else{
                $number=6;$maxNumber=8;
                ...
                $name_length=mb_strlen($name,'utf-8');
                if($number>$name_length){
                    $orderInfo.='<L1><B>'.self::LR($name,' ×'.$num,16).'</B></L1>';
                }else{
                    $orderInfo.='<L1><B>'.self::LR(mb_substr($name,0,$number),' ×'.$num,16).'</B></L1>';
                }
                $remainder= ceil((mb_strlen($name,'utf-8')-$number) /$maxNumber);
                if($remainder>0){
                    for ($i=0;$i<$remainder;$i++){
    ...
    {
        $content = '<L1><center>退菜单</center></L1><BR>';
        if ($order->tableId) {
            if($order->table->area->name){
                $content .= '<L1>桌号:' . $order->table->area->name . '-' . $order->table->name . '</L1><BR>';
            }else{
                $content .= '<L1>桌号:' .  $order->table->name . '</L1><BR>';
            }

            $content .= '人数：' . $order->people . '<BR>';
        } else {
            $content .= '<L1>牌号:' . $order->pickNo . '</L1><BR>';
        }
        $content .= '类型：' . $order->packagingFormat . '<BR>';
        $content .= '菜品                        数量<BR>';
        $content .= '--------------------------------<BR>';
        if ($orderGoods) {
            ...
            $content .= '<L1>' . self::strFix($name, 31) .    self::strFix($orderGoods['num'], 1)  . '</L1>';
        } else {
            ...
            $content .= '<L1>' . self::feiStyle($goods, 2, '(退)') . '</L1>';
        }
        $content .= '--------------------------------<BR>';
        if ($order->notes) {
            $content .= '<H><B>整单备注：' . $order->notes . '</H></BOLD><BR>';
            $content .= '--------------------------------<BR>';
        }
    ...
    public static function turntable($fromTableId,$toTableId,$order,$operator=''){
        $content = '<L1><C>转台单</C></L1>';
        $content .= '--------------------------------<BR>';
        $from=Table::with('type','area')->where('id',$fromTableId)->first();
        $to=Table::with('type','area')->where('id',$toTableId)->first();
        $content .='<H><B>'.self::LR('原桌位：', $from->area->name.'-'.$from->type->name.'-'.$from->name, 32) . '</B></H><BR>';
        $content .= '<H><B>'.self::LR('转桌位：', $to->area->name.'-'.$to->type->name.'-'.$to->name, 32) . '</B></H><BR>';
    ...
    public static function handoverNote($handoverOrder){
        $content = "<L1><C>".$handoverOrder->store->name."</C></L1>";
        $content.= "<L1><C>【交班单】</C></L1>";
        $content .= "交班班次：午班<BR>";
...
//        $content .= '快餐            '.$handoverOrder->contents->unclosedFast->count.'单       '.$handoverOrder->contents->unclosedFast->money.'<BR>';
    ...
    public static function queuingNumber ($queueOrder){
        ...
        $content .= '<QRCODE>'.$queueOrder->qrcode.'</QRCODE><BOLD>还等几桌？扫码查进度</BOLD><BR>';
