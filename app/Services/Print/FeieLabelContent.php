...
class FeieLabelContent
{
    ...
    static function feiStyleLabel($goods, $order, $printData, $inedx=1, $num=1)
    {
        $data = [];
        $y = 10;
        $content = "";
        ...
        if ($y <= 210) {
            $time = $order->serverTime ?: date("Y-m-d H:i:s", time());
            $time = Carbon::createFromFormat("Y-m-d H:i:s", $time)->format('m-d H:i');
            $diningTypeFormat = $order->diningTypeFormat;
            if ($order->packaging == 1) {
                $diningTypeFormat = '打包带走';
            }
            $content .= '<TEXT x="10" y="210" font="12" w="1" h="1" r="0">' . $time . " " . $diningTypeFormat . '</TEXT>';
        }
        $data[] = $content;
        return $data;
        //
