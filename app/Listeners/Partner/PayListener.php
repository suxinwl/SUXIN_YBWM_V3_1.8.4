...
class PayListener implements ShouldQueue
{
    ...
    public function handle(PartnerEvent $event)
    {
        try {
            $order = $event->model;
            $partners  = PartnerOrder::where('orderSn', $order->orderSn)->where('isPay',0)->get();
            if ($partners) {
                foreach ($partners as $key => $partnerOrder) {
                    $res =  MemberAccountService::freezeAmount($partnerOrder->partnerId, 1, $partnerOrder->money, MemberAccountLog::FREEZEAMOUNT_PARTNER_ADD, 0, $partnerOrder->orderSn, '');
                    if ($res) {
                        $partnerOrder->state = $order->state;
                        $partnerOrder->isPay = 1;
                        $partnerOrder->save();
                    }
                }
            }
    ...
    {
        return $event->model->state >= 2 && $event->model->state < 6;
    }
}
