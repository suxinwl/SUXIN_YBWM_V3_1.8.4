...
use App\Exports\OrderCollectReceiveExport;
use App\Exports\OrderCollectReceiveListExport;
use App\Http\Controllers\Channel\ApiController;
use App\Models\OrderCollect\OrderCollect;
use App\Models\OrderCollect\Receive;
use App\Models\OrderCollect\Stores as OrderCollectStores;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;

class OrderCollectController extends ApiController
{
    /**
    ...
    {
        $list = OrderCollect::where('uniacid', $this->uniacid())
    ...
    public function store(Request $request)
    {
        try {
            ...
            $ids = OrderCollect::where('uniacid', $this->uniacid())
            ...
            OrderCollectStores::where('collectId', $model->id)->update(['uniacid' => $this->uniacid(), 'type' => $model->storeType]);
    ...
    {
        try {
            $model = OrderCollect::where("uniacid", $this->uniacid())->find($id);
            if (empty($model)) {
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            $models = OrderCollect::where('uniacid', $this->uniacid())->whereIn('id', $idArray)->get();
    ...
    {
        $model = OrderCollect::where('id', $id)->first();
        if (!$model) {
    ...
    {
        $list = Receive::with(['order', 'member', 'ordercollect'])
            ...
            ->when($request->startTime && $request->end, function ($q) use ($request) {
                return $q->where('created_at', '>=', $request->startTime)
                    ->where('created_at', '<=', $request->endTime);
            })
            ->when($request->activiryId, function ($q) use ($request) {
                return $q->where('collectId', $request->activiryId);
            })
            ->orderBy('id', 'desc')
            ->paginate(Request()->pageSize ?? 20, '*', 'pageNo');
        return $this->success($list);
    ...
    public function orderDataExport(Request $request)
    {
        ...
        if ($params['type'] == 'receive') {
            return Excel::download(new OrderCollectReceiveListExport($params), 'orderCollectReceiveListExport.xlsx');
        } else {
            return Excel::download(new OrderCollectReceiveExport($params), 'orderCollectReceiveExport.xlsx');
        }
    }
}
