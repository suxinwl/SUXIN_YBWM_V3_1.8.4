...
class GooodsDiscountController extends ApiController
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        foreach ($model->goodsData['goods'] as $key => $goods) {
            ...
            $goods['discountLabel'] = $model->discountLabel;
            $goods['uniacid'] = $model->uniacid;
            $goods['scenario'] = json_encode($model->scenario, 320);
            $goods['rule'] = empty($model->rule) ? null : json_encode($model->rule, 320);
            $goods['discountRule'] = empty($model->discountRule) ? null : json_encode($model->discountRule, 320);
            unset($goods['name'], $goods['price'], $goods['id']);
            $goodslist[] = $goods;
            $spuIds[] = $goods['spuId'];
        }
        if ($goodslist) {
