...
class GoodsController extends ApiController
{
    public function index(Request $request)
    {
        try {
            ...
            // ->increment('pv', 1);
            return $this->success(new GoodsGoodsList($list, $data, $userId = 0));
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // StatisticsDay::where('uniacid', $this->uniacid())->where('day', date("Y-m-d", time()))->increment('pv', 1);
            ...
            ->when($request->keyword, function ($q) use ($request, $uniacid) {
                return $q->whereHas('spu', function ($q) use ($request, $uniacid) {
                    return $q->where(function ($q) use ($request) {
                        return $q->where('name', 'like', "%$request->keyword%")->orWhere("pinYin", 'like', "%$request->keyword%");
